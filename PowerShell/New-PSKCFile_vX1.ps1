[CmdletBinding()]
param([string]$Manufacturer, [string]$SerialNo, [string]$User="", [int]$OTPLength=6, [string]$OTPType="HOTP", $TestMode = $false, [string]$KeyType = "PreSharedKey")
...
####### Generates a random password; either a byte array or text depending on the value of "Initialisation"
    if($Initialisation -match "Seed") {
        if ($Initialisation -match "Test") {
            ...
            return [Text.Encoding]::UTF8.GetBytes("12345678901234567890")
        } else {
            $bufferPWd = New-Object byte[] ($Length)
            $randy = New-Object System.Security.Cryptography.RNGCryptoServiceProvider
...
if($KeyType -match "PreSharedKey"){
    Write-Verbose "Creating PSKC file encrypted with Pre-Shared Key for device $($Manufacturer):$($SerialNo). Type=$OTPType, $OTPLength digits"
} else {
    Write-Verbose "Creating PSKC file encrypted with PBKDF2 (passphrase-derived key) for device $($Manufacturer):$($SerialNo). Type=$OTPType, $OTPLength digits"
}
$Mode = ""
...
if ($OTPType -eq "HOTP") {
    ...
    Write-Error "OTPType  TOTP is not supported"; Return 
} else {
...
$OutputFileName = Join-Path $PSScriptRoot ("$Manufacturer_xxx_$($User).pskc")
...
####### Put the results into the XML Doc for output
...
if($SerialNo.Length -gt 0) {
    ...
    $OutputFileName = Join-Path $PSScriptRoot ("$Manufacturer_$($SerialNo)_$($User).pskc")
}
if($KeyType -match "PreSharedKey"){
...
$OTPURI = "otpauth://hotp/GRHOTP:$($User)?secret=$($SeedB32)&counter=0&digits=$($OTPLength)&issuer=GResearch"
$OutputPNGFileName = $OutputFileName.Replace('.pskc','.png')
Write-Host "Writing URI to QRCode file $OutputPNGFileName"
Write-Host "Writing PSKC to XML file   $OutputFileName"
Write-Host " URI:            $OTPURI"
...
if($KeyType -match "PreSharedKey"){ 
    Write-Host " Pre-Shared Key:" (ConvertTo-HexString $Key) 
} else {
}
